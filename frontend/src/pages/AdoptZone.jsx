...
const AdoptZone = () => {
  ...
  useEffect(() => {
    const fetchZoneDetails = async () => {
      try {
        const response = await fetch('https://biorevive-backend-6yij.onrender.com/api/zones');
        if (response.ok) {
  ...
  const handleAdoptSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        await fetch(`https://biorevive-backend-6yij.onrender.com/api/zones/${id}/adopt`, {
          method: 'POST',
