...
const Dashboard = () => {
      ...
      try {
        ...
        if (isMounted) {
          ...
          if (recentZones.length > 0) {
            setSelectedZoneId((prev) => prev || recentZones[0].id);
          }
        }
  ...
  const handleAddReport = async (e) => {
    ...
    try {
      ...
      await submitReport(newReportData);
      ...
      const refreshedReports = await fetchReports();
      setReports([...refreshedReports].reverse().slice(0, 3));
      setStats((prev) => ({ ...prev, treesPlanted: refreshedReports.length || 0 }));
    } catch (error) {
    ...
    <div className="relative transition-all duration-300 w-full bg-[#F8FAFC]">
      ...
      <div className="block md:hidden min-h-screen bg-[#F8FAFC] pb-24 font-sans">
        <div className="bg-[#114A29] text-white px-6 pt-6 pb-8 rounded-b-3xl shadow-md">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <h1 className="text-2xl font-extrabold">Hello, {profile?.full_name?.split(' ')[0] || 'User'} 👋</h1>
            </div>
        ...
        <div className="px-4 -mt-4 space-y-5">
          {/* Mobile Live IoT Telemetry Card */}
                  ...
                  <div key={zone.id} onClick={() => navigate('/map')} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between cursor-pointer hover:border-green-200 transition">
                    <div className="flex items-center space-x-3">
                      <div className="w-12 h-12 rounded-xl bg-green-50 border border-green-100 flex items-center justify-center text-[#114A29] font-bold text-xs uppercase shadow-sm">
                        {zone.status ? zone.status.substring(0, 2) : 'ZN'}
                      </div>
                      <div>
      ...
      <div className="hidden md:block p-8 min-h-screen font-sans w-full">
        ...
        {/* 🟢 Live IoT Telemetry Widget (Desktop) */}
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-3">
              <h3 className="font-extrabold text-lg flex items-center gap-2">
                🌱 Live IoT Sensor Stream (Java Backend)
              </h3>
              {/* 🌦️ Live Season Badge */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/10">
              ...
              <p className="text-2xl font-black mt-1">
                {telemetry ? `${telemetry.soilMoisture.toFixed(2)}%` : 'Loading...'}
              </p>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/10">
              ...
              <p className="text-2xl font-black mt-1">
                {telemetry ? `${telemetry.temperature.toFixed(2)} °C` : 'Loading...'}
              </p>
            </div>

            <div className="bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/10">
              <p className="text-xs text-green-200 font-bold uppercase">NPK Status</p>
              <p className="text-sm font-bold mt-2 text-green-100">
                {telemetry ? telemetry.npkLevel : 'Loading...'}
              </p>
            </div>
          </div>
        </div>

        {/* 🤖 Desktop Swarm AI Multi-Agent Insights Card */}
        <div className="mb-8">
          <SwarmInsightsCard insights={swarmInsights} />
        </div>
