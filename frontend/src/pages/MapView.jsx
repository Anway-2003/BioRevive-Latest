...
import { Search, X, ChevronDown, MapPin, Filter, Layers, Navigation, ChevronRight, Map as MapIcon } from 'lucide-react';
...
const MapView = () => {
  ...
  // 🚀 Map Center & Search States
  const [mapCenter, setMapCenter] = useState([19.6156, 77.7963]); // Default Location
  const [searchQuery, setSearchQuery] = useState(''); // Search bar sathi
  ...
  useEffect(() => {
    ...
    // LOCATION FETCH KARNYACHA LOGIC (Works best on HTTPS / Real Mobile App)
  ...
  // 🚀 NAVIN FUNCTION: Search Bar kam karnyasathi
  const handleSearch = async (e) => {
    if (e) e.preventDefault();
    if (!searchQuery.trim()) return;

    try {
      // OpenStreetMap chi free API search karnyasathi
      const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(searchQuery)}`);
      const data = await response.json();

      if (data && data.length > 0) {
        // Location milali! Map tithhe fly karel
  ...
  // 🚀 NAVIN FUNCTION: Button click karun parat current location var yenyasathi
  ...
  const handleSaveZone = async (e) => {
    ...
    try {
      ...
      setZoneName('');
      fetchZones(); 
      alert('🔥 Navin Zone Database madhe Save zala!');
    } catch (error) {
    ...
    <div className="w-full">
      ...
      <div className="block md:hidden min-h-screen bg-[#F8FAFC] pb-24 font-sans relative flex flex-col">
        ...
        <div className="bg-[#114A29] text-white px-4 pt-5 pb-4 shadow-md z-10">
          ...
          <div className="relative flex items-center">
            {/* 🚀 Mobile Search Input Binded to State */}
            <input 
              type="text" 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
              placeholder="Search Yavatmal, Pune..." 
        ...
        {/* Real Interactive Map for Mobile */}
        ...
        <div className="px-4 py-4 space-y-3 bg-white rounded-t-3xl -mt-6 shadow-lg relative z-20 min-h-[300px]">
          ...
          <h3 className="text-xs font-bold uppercase tracking-wider text-gray-400 px-1">Active Zones in Database</h3>
          
          {loading ? (
            <div className="text-center text-xs font-bold text-gray-400 mt-5">Loading...</div>
          ) : zones.map((zone) => {
            const styles = getZoneStyles(zone.status);
            return (
              <div key={zone.id} onClick={() => { setMapCenter([zone.latitude, zone.longitude]); }} className="p-3 bg-gray-50 rounded-2xl border border-gray-100 flex items-center justify-between cursor-pointer hover:bg-green-50/50 transition">
      ...
      <div className="hidden md:flex flex-row h-screen bg-gray-50 overflow-hidden font-sans w-full relative">
        ...
        <div className="flex-1 relative z-0">
          ...
          {/* 🚀 Floating Top Search Bar for Desktop */}
          <div className="absolute top-6 left-6 z-[400] bg-white rounded-xl shadow-md px-4 py-3 flex items-center gap-3 w-80 md:w-96 border border-gray-100">
            <input 
              ...
              onChange={(e) => setSearchQuery(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
              placeholder="Search Yavatmal, Pune, etc..." 
            ...
            {/* Locate Me Button inside search bar */}
        ...
        <div className="w-80 bg-white shadow-[-10px_0_20px_-10px_rgba(0,0,0,0.1)] z-10 flex flex-col border-l border-gray-100 overflow-y-auto">
          <div className="p-6">
            ...
            <h4 className="font-bold text-sm text-gray-800 mb-3">Live Database List</h4>
            <div className="space-y-3 mb-8">
               {loading ? (
                 ...
                 const styles = getZoneStyles(zone.status);
                 return (
                  <div key={zone.id} onClick={() => setMapCenter([zone.latitude, zone.longitude])} className="p-3 bg-gray-50 border border-gray-100 rounded-xl cursor-pointer hover:bg-green-50 transition">
                     <h4 className="font-bold text-gray-900 text-sm">{zone.name}</h4>
          ...
          <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl relative">
            ...
            <form onSubmit={handleSaveZone} className="space-y-4">
              ...
              <button type="submit" disabled={submitting} className="w-full bg-[#114A29] hover:bg-green-900 text-white font-bold py-3.5 rounded-xl transition shadow-md mt-4 disabled:opacity-70">
                {submitting ? 'Saving to Database...' : 'Save Zone'}
              </button>
            </form>
          </div>
        </div>
