...
const Dashboard = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      let fetchedReports = [];
      try {
        const repResponse = await fetch('http://https://biorevive-backend-6yij.onrender.com/api/reports');
        if (repResponse.ok) {
        ...
        <div className="bg-[#114A29] text-white px-6 pt-6 pb-8 rounded-b-3xl shadow-md">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <h1 className="text-2xl font-extrabold">Hello, {profile?.full_name?.split(' ')[0] || 'Anway'} 👋</h1>
            </div>
